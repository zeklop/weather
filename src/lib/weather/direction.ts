...
/** Compass label for wind direction (direction the wind blows FROM), 8 Russian points. */
export function windDirectionLabel(degrees: number): string {
	// Round to the nearest 45° sector, north-centered at 0°/360°.
	const sectorIndex = Math.floor((normalizeDegrees(degrees) + SECTOR / 2) / SECTOR) % DIRECTIONS.length;
