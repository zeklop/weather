...
| Time handling | `response.timezone` stored in payload. Open-Meteo values are wall-time ISO in location tz; work only with wall-time, never `new Date()` conversion. HH:MM via ISO string slice; date/weekday via `Intl` + `Date.UTC`/`timeZone:'UTC'` trick (wall-time treated as UTC — no phone-tz shift). `isDay(at, sunrise, sunset): DayResult` = `{ isDay: boolean; source: 'calculated' \| 'fallback' }`: missing sunrise/sunset → fallback window 07:00–19:00, `source:'fallback'`; `R<S` normal → `R<=T<=S`; `R==S` degenerate → day; `R>S` cross-midnight → `T>=R \|\| T<=S`. Tests: day/night/boundaries, fallback-window boundaries (06:59/07:00/18:59/19:00), cross-midnight, degenerate, missing values, DST, foreign tz | Spec §23; never phone-local tz |
| App icon safe-zone | Numeric invariant, enforced by `scripts/check-icons.mjs` in CI: all non-transparent pixels of maskable icon inside central circle radius `0.4 * size` (512 → ≤204.8 px); exact sizes 180/192/512 verified | Reproducible, verifiable |
