...
| Time handling | `response.timezone` stored in payload. Open-Meteo values are wall-time ISO in location tz; work only with wall-time, never `new Date()` conversion. HH:MM via ISO string slice; date/weekday via `Intl` + `Date.UTC`/`timeZone:'UTC'` trick (wall-time treated as UTC — no phone-tz shift). `isDay(at, sunrise, sunset)` rules: missing sunrise/sunset → fallback window 07:00–19:00 (`source:'fallback'`); `R<S` normal → `R<=T<=S`; `R==S` degenerate → day; `R>S` cross-midnight → `T>=R \|\| T<=S`. Tests: day/night/boundaries, cross-midnight, degenerate, missing values, DST, foreign tz | Spec §23; never phone-local tz |
